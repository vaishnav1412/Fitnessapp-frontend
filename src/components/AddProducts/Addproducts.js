...
const Addproducts = () => {
  ...
  const handleSubmit = async (event)=>{ 
    ...
    if (sellingPrice==='') {
      setSellingPriceError("Please enter price ");
      isVarified = false;
    }
    if (quantity==='') {
    ...
    if (isVarified) {
      ...
      try {
        admininstance
        .post(apiEndPoints.addProducts,formData, {
          headers:{
  ...
  return (
    ...
    <div class="register-form bg-white bg-opacity-70 rounded-lg shadow-lg p-6 w-full sm:w-96">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <input
            className="input-register border rounded-lg p-2 w-full"
            placeholder="Enter Products Actual  Price"
            name="productprice"
            type='number'
        ...
        <input
            className="input-register border rounded-lg p-2 w-full"
            placeholder="Enter Products Selling  Price"
            name="productprice"
            type='number'
            onChange={handleSellingPrice}
          />
          <p className="text-red-500">{SellingPriceError}</p>
        </div>
